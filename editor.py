...
class SaveEditor(QMainWindow, MainWindow):
    ...
    def setup_ui(self):
        ...
        self.floor_widget.itemClicked.connect(self.change_selectmap)
    ...
    def replace_map_pic(self):
        src_num, ok = QInputDialog.getInt(
            self, "替换所有图块", "请输入需要替换的图块编号：(1-137)"
        )
        if not ok:
            return
        tag_num, ok = QInputDialog.getInt(
            self, "替换所有图块", "请输入要替换为图块编号：(1-137)"
        )
        if not ok:
            return
        for label_widget in self.label_list:
...
class Setting(QDialog, SettingWindow):
    ...
    def set_args(self):
        self.key_widgets_default = {
            ...
            "save1nod": self.nod_edit,  # 任意门
            "save1nok": self.nok_edit,  # 神剑之证
            "save1bigchor": self.bigchor_edit,  # 宝石锄头
            "save1life": self.relive_edit,  # 复活十字
            "save1fly": self.fly_check,  # 楼层传送器
            "save1ddd": self.ddd_check,  # 怪物手册
            "save1ice": self.ice_check,  # 冰之护符
            "save1betterdf": self.magicdef_check,  # 魔法护符（降低魔法师防御的道具，商人出售）
            "save1chor": self.chor_check,  # 小偷锄头（20F假公主掉落）
            "save1key": self.key_check,  # 仙子钥匙（-2F对话获得）
            # =========================================
            # 存档相关
            # =========================================
            ...
            "save1svdata": self.savename_edit,  # 存档名称
            # =========================================
            # 战斗相关
            # =========================================
            "save1attimes": self.atktimes_edit,  # 每回合攻击次数
            "save1heavyda": self.baoji_edit,  # 暴击几率%
            "save1nearat": self.atknear_edit,  # 攻击临界值
            "save1neardf": self.defnear_edit,  # 防御临界值
            # =========================================
            # 杂项相关
            # =========================================
            "save1need1": self.shop3f_edit,  # 3F商店收费
            "save1need2": self.shopb5f_edit,  # -5F商店收费
            "save1maxgou": self.minf_edit,  # 最高可飞楼层
            "save1maxgoo": self.maxf_edit,  # 最低可飞楼层
            # =========================================
            # 章相关
            "save1intter": self.intter_edit,  # 贤者变量为：60（额外+5敏捷）、80、100
            "save1king": self.king_edit,  # 霸者变量为：10（额外+15攻防）、20、30
            "save1brave": self.brave_edit,  # 勇者变量为：1（暴击率+5%）、2（攻击次数+1）、3
            # =========================================
            # 尚未探明的变量
            # =========================================
        }
